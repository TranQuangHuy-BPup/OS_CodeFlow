<div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm mb-8" x-data="{ algo: '{{ $algorithm ?? 'fcfs' }}' }">
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
        ...
        <div class="md:col-span-1" x-show="algo === 'round_robin'" x-cloak>
            ...
            <input type="number" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-slate-700 font-mono focus:ring-2 focus:ring-primary/20 outline-none transition-all" value="2">
        </div>
        {{-- Action Buttons (Cố định vị trí bên phải) --}}
        <div class="col-span-1 md:col-start-3 md:col-span-2 flex gap-3 justify-end">
            <button class="bg-primary text-white px-6 py-2.5 rounded-xl font-bold hover:opacity-90 active:scale-95 transition-all flex items-center gap-2 shadow-md">
                <span class="material-symbols-outlined text-base">play_arrow</span> Simulate
            </button>
